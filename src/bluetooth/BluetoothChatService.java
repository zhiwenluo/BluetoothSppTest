...
public class BluetoothChatService {
	...
	mHandler.obtainMessage(StringConstant.MESSAGE_STATE_CHANGE, state, -1)
		.sendToTarget();
    }
	...
	Message msg = mHandler
		.obtainMessage(StringConstant.MESSAGE_DEVICE_NAME);
	Bundle bundle = new Bundle();
	bundle.putString(StringConstant.DEVICE_NAME, device.getName());
	msg.setData(bundle);
	...
	Bundle bundle = new Bundle();
	bundle.putString(StringConstant.TOAST, "Unable to connect device");
	msg.setData(bundle);
	mHandler.sendMessage(msg);
	...
	Message msg = mHandler.obtainMessage(StringConstant.MESSAGE_TOAST);
	Bundle bundle = new Bundle();
	bundle.putString(StringConstant.TOAST, "Device connection was lost");
	msg.setData(bundle);
	mHandler.sendMessage(msg);
	...
	public void run() {
		...
		try {
		    ...
		    Message msg = mHandler
			    .obtainMessage(StringConstant.MESSAGE_TOAST);
		    Bundle bundle = new Bundle();
	...
	private Boolean first0xAA = false;
	...
	public void run() {
		...
		try {
			...
			if (start) {
				...
				// 开始接收帧数据
				receivedBuffer[receivePosition] = length;
				receivePosition++;
				if (receivePosition >= frameLength) {
				    // 接收完毕
				    byte[] realBuffer = new byte[frameLength];
				    System.arraycopy(receivedBuffer, 0,
					    realBuffer, 0, frameLength);
				    mHandler.obtainMessage(
					    StringConstant.MESSAGE_READ,
					    frameLength, -1, realBuffer)
					    .sendToTarget();
				    receivePosition = 0;
				    start = false;
				    frameLength = -1;
	...
	public void write(byte[] buffer) {
		...
		mHandler.obtainMessage(StringConstant.MESSAGE_WRITE, -1, -1,
			buffer).sendToTarget();
	    } catch (IOException e) {
